...
保留期默认 7 天。App 会先完成初始日志读取、状态计算以及窗口和托盘初始化，再在后台进行过期清理；两秒是发出取消的尽力工作预算，不代表 Windows 的同步文件删除能被瞬间中断。清理失败或超出预算不会回头阻塞启动，也不会显示内部路径或错误；退出 App 时会请求取消，但不会无限等待仍在进行的文件删除。清理与界面读取共用日志互斥体，因此后台清理尚未结束时，之后手动刷新记录可能短暂等待。离开 Detailed 时，界面会询问是否删除已有详细日志。
